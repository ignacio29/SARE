...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 21-11-2017 a las 07:29:48
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarComentario` (IN `idnoticia` INT(11), IN `cedula` VARCHAR(100), IN `comentario` TEXT, IN `fecha` DATE)  MODIFIES SQL DATA
INSERT INTO tbcomentarionoticia(idnoticia,cedularesponsable,comentario,fechacomentario) VALUES(idnoticia,cedula,comentario,fecha)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `insertartNoticia` (IN `creador` VARCHAR(50), IN `tema` VARCHAR(100), IN `descripcion` TEXT, IN `rutaFoto` VARCHAR(100), IN `fechaCreacion` DATE)  MODIFIES SQL DATA
