...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 15-03-2018 a las 10:40:29
...
-- Procedimientos
--
CREATE  PROCEDURE `actualizarAsistenteAdmi` (IN `nombre` VARCHAR(20), IN `primerA` VARCHAR(20), IN `segundoA` VARCHAR(20), IN `sexo` VARCHAR(20), IN `cedula` VARCHAR(20))  MODIFIES SQL DATA
...
CREATE  PROCEDURE `actualizarEstudianteAdmi` (IN `cedula` VARCHAR(20), IN `nombre` VARCHAR(20), IN `pApellido` VARCHAR(20), IN `sApellido` VARCHAR(20), IN `sexo` VARCHAR(10), IN `cabina` INT, IN `fecha` INT, IN `carrera` VARCHAR(50), IN `acceso` INT)  READS SQL DATA
...
CREATE  PROCEDURE `actualizarNoticia` (IN `creador` VARCHAR(100), IN `tema` VARCHAR(100), IN `detalle` TEXT, IN `rutaFoto` VARCHAR(100), IN `id` INT(11))  MODIFIES SQL DATA
...
CREATE  PROCEDURE `autenticarCuenta` (IN `usuario` VARCHAR(100), IN `contrasena` VARCHAR(100))  READS SQL DATA
SELECT * FROM tblogin WHERE userlogin=usuario AND passwordlogin=contrasena$$

CREATE  PROCEDURE `consultarAdministradorLogin` (IN `idLogin` INT)  NO SQL
SELECT * FROM tbadministrador WHERE loginid=idLogin$$

CREATE  PROCEDURE `consultarAsistenteLogin` (IN `idLogin` INT)  NO SQL
SELECT * FROM tbasistente WHERE loginid=idLogin$$

CREATE  PROCEDURE `consultarEstudiante` (IN `idEstudiante` INT(10))  READS SQL DATA
SELECT cedulaestudiante FROM tbestudiante WHERE idestudiante=idEstudiante$$

CREATE  PROCEDURE `consultarEstudianteLogin` (IN `idLogin` INT)  READS SQL DATA
SELECT cedulaestudiante FROM tbestudiante WHERE loginestudiante=idLogin$$

CREATE  PROCEDURE `consultHorario` (IN `cedula` VARCHAR(30))  READS SQL DATA
...
CREATE  PROCEDURE `deleteHorario` (IN `id` INT)  MODIFIES SQL DATA
DELETE FROM tbhorario WHERE idhorario=id$$

CREATE  PROCEDURE `elimarAsistente` (IN `cedula` VARCHAR(50))  MODIFIES SQL DATA
DELETE FROM tbasistente WHERE cedulaasistente=cedula$$

CREATE  PROCEDURE `elimarEstudiante` (IN `cedula` VARCHAR(50))  MODIFIES SQL DATA
DELETE FROM tbestudiante WHERE cedulaestudiante=cedula$$

CREATE  PROCEDURE `eliminarComentariosNoticia` (IN `idnoticia` INT(11))  MODIFIES SQL DATA
DELETE FROM tbcomentarionoticia WHERE idnoticia=idnoticia$$

CREATE  PROCEDURE `eliminarNoticia` (IN `id` INT(11))  MODIFIES SQL DATA
DELETE FROM tbnoticias WHERE idnoticia=id$$

CREATE  PROCEDURE `getIdConfirmarHorario` (IN `cedula` VARCHAR(100))  READS SQL DATA
SELECT confirmahorarioestudiante FROM tbestudiante WHERE tbestudiante.cedulaestudiante=cedula$$

CREATE  PROCEDURE `getIdEstudiante` (IN `cedula` VARCHAR(100))  READS SQL DATA
SELECT tbestudiante.idestudiante FROM tbestudiante WHERE tbestudiante.cedulaestudiante=cedula$$

CREATE  PROCEDURE `getIndiceImagen` (IN `creador` VARCHAR(100))  READS SQL DATA
SELECT * FROM tbnoticias WHERE idcreadornoticia=creador$$

CREATE  PROCEDURE `insertarComentario` (IN `idnoticia` INT(11), IN `cedula` VARCHAR(100), IN `comentario` TEXT)  MODIFIES SQL DATA
INSERT INTO tbcomentarionoticia(idnoticia,cedularesponsable,comentario) VALUES(idnoticia,cedula,comentario)$$

CREATE  PROCEDURE `insertarNoticia` (IN `creador` VARCHAR(100), IN `tema` VARCHAR(100), IN `descripcion` TEXT, IN `rutaFoto` VARCHAR(100), IN `fechaCreacion` DATE)  MODIFIES SQL DATA
...
CREATE  PROCEDURE `insertarPermiso` (IN `creador` VARCHAR(50), IN `asunto` VARCHAR(100), IN `descripcion` TEXT, IN `fecha` DATE)  MODIFIES SQL DATA
...
CREATE  PROCEDURE `insertAsistente` (IN `name` VARCHAR(50), IN `primerA` VARCHAR(50), IN `segunA` VARCHAR(50), IN `cedul` VARCHAR(50), IN `idL` INT, IN `sexo` VARCHAR(50))  NO SQL
...
CREATE  PROCEDURE `insertEstudiante` (IN `nombreE` VARCHAR(50), IN `primerApe` VARCHAR(50), IN `segundoApe` VARCHAR(50), IN `cedulaE` VARCHAR(50), IN `cabinaE` INT(50), IN `carreraE` VARCHAR(50), IN `logE` INT, IN `confCuen` TINYINT, IN `confHora` TINYINT, IN `sexo` VARCHAR(50), IN `ingreso` INT)  NO SQL
...
CREATE  PROCEDURE `insertHorario` (IN `idestudiante` BIGINT, IN `dia` TEXT, IN `horaInicio` VARCHAR(10), IN `horaFinal` VARCHAR(10), IN `descripcion` VARCHAR(100))  NO SQL
...
CREATE  PROCEDURE `insertLogin` (IN `usuario` VARCHAR(100), IN `contrasena` VARCHAR(100), IN `rol` VARCHAR(30))  MODIFIES SQL DATA
INSERT INTO tblogin (userlogin,passwordlogin,rollogin) VALUES(usuario,contrasena,rol)$$

CREATE  PROCEDURE `marcarVisto` (IN `id` INT(11))  NO SQL
update tbpermisos set visto=1 where idpermiso=id$$

CREATE  PROCEDURE `mostrarComentarioAvisoAdministrador` (IN `idaviso` INT(11))  READS SQL DATA
...
CREATE  PROCEDURE `mostrarComentarioAvisoAsitente` (IN `idaviso` INT(11))  READS SQL DATA
...
CREATE  PROCEDURE `mostrarComentarioAvisoEstudiante` (IN `idaviso` INT(11))  READS SQL DATA
...
CREATE  PROCEDURE `mostrarMisAvisos` (IN `creador` VARCHAR(100))  MODIFIES SQL DATA
SELECT * FROM tbnoticias WHERE idcreadornoticia=creador$$

CREATE  PROCEDURE `mostrarMisPermisos` (IN `cedula` VARCHAR(100))  READS SQL DATA
SELECT * FROM tbpermisos WHERE creadorpermiso=cedula ORDER BY visto ASC,fechapermiso DESC$$

CREATE  PROCEDURE `mostrarTodasNoticiasAdministrador` ()  READS SQL DATA
...
CREATE  PROCEDURE `mostrarTodasNoticiasAsistente` ()  MODIFIES SQL DATA
...
CREATE  PROCEDURE `mostrarTodosPermisos` ()  READS SQL DATA
...
CREATE  PROCEDURE `updateAsistente` (IN `name` VARCHAR(50), IN `primeA` VARCHAR(50), IN `segunA` VARCHAR(50), IN `cedul` VARCHAR(50), IN `sex` VARCHAR(50), IN `idAsis` INT(11))  MODIFIES SQL DATA
...
CREATE  PROCEDURE `updateConfirmarHorario` (IN `cedula` VARCHAR(100))  MODIFIES SQL DATA
UPDATE tbestudiante SET confirmahorarioestudiante=1 WHERE cedulaestudiante=cedula$$

CREATE  PROCEDURE `updateEstudiante` (IN `name` VARCHAR(50), IN `primeA` VARCHAR(50), IN `segunA` VARCHAR(50), IN `cabina` INT(10), IN `carrera` VARCHAR(50), IN `cedul` VARCHAR(50), IN `sex` VARCHAR(50), IN `ingreso` INT(10), IN `idEst` INT(10))  MODIFIES SQL DATA
...
CREATE  PROCEDURE `updateHorario` (IN `dia` TEXT, IN `horaInicio` VARCHAR(10), IN `horaFin` VARCHAR(10), IN `id` INT, IN `descripcion` VARCHAR(100))  MODIFIES SQL DATA
...
CREATE  PROCEDURE `verAsistentes` ()  READS SQL DATA
...
CREATE  PROCEDURE `verDatosAsistente` (IN `id` INT(10))  READS SQL DATA
...
CREATE  PROCEDURE `verDatosEstudiante` (IN `id` INT(10))  READS SQL DATA
...
CREATE  PROCEDURE `verEstudiantes` ()  READS SQL DATA
...
CREATE  PROCEDURE `verificarEstadoArea` (IN `idArea` INT(11))  NO SQL
Select * FROM tbareaslimpieza WHERE tbareaslimpieza.idarea=idArea and tbareaslimpieza.estado=1$$

CREATE  PROCEDURE `verificarHorarioEstudiante` (IN `cedula` VARCHAR(50), IN `dia` VARCHAR(50), IN `horaInicio` VARCHAR(50), IN `horaSalida` VARCHAR(50))  READS SQL DATA
...
INSERT INTO `tbarticulos` (`nombrearticulo`, `idarticulo`, `seriearticulo`, `idpropietario`, `tipoarticulo`, `descripcionarticulo`, `estadoarticulo`, `clasearticulo`) VALUES
...
('rwerwerwer', 3, 435345, 115700773, 'Cuidado Personal', 'DSDS', 'Buen estado', ''),
('eqweqw', 4, 0, 115700773, 'Electronico', 'qweqweqwe', 'Regular', ''),
('rwrwer', 7, 0, 115700773, 'Electronico', 'werwer', 'Deficiente', 'Electronico');
...
(100, '777777777777', 'luis', 'parra', 'cambronero', 'M', 10);
...
(38, '1312312312321', 'eqw', 'htgjhg', '  jghj', 'M', 1, 12, 'Administracion de empresas', 43, 0, 0),
...
--
ALTER TABLE `tbestudiante`
  MODIFY `idestudiante` bigint(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=44;
--
